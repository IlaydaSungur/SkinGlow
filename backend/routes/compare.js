...
const { getEmbeddingsBatch, cosineSimilarity } = require("../services/compareService");
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!shelfData || shelfData.length === 0) {
      return res.json({ matches: [], productSimilarities: [], message: "Shelf is empty" });
    }
    ...
    for (const row of shelfData) {
      ...
      for (let i = 0; i < ingredients.length; i++) {
        for (let j = 0; j < shelfIngredients.length; j++) {
          ...
          console.log(`🔍 ${ingredients[i]} vs ${shelfIngredients[j]} → ${percentScore}%`);
      ...
      if (matchCount > 0) {
        ...
        console.log(`   Benzer içerikler:`, productMatches.map(m => `${m.ingredient} ↔ ${m.shelfItem}`).join(", "));
      } else {
